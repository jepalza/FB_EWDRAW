' for AxSuite version 3.2.0.0  date: 23-Feb-2015
...
' jepalza, tapo esto, molesta al compilar
'	#print ====
...
'   #print ====
   ...
   #Ifndef Ax_NoAtl
      ...
      ' jepalza: añadido "or WS_child" para que sea HIJO-DE...
      FUNCTION AxWinFull(byVal h_parent as hwnd, name1 as string, progid as string, _
               x as integer, y as integer, w as integer, h as integer, _
               style as integer = WS_visible or WS_OVERLAPPEDWINDOW or WS_child or WS_border, exstyle as integer = 0) as hwnd
         Dim as hwnd h1
         h1 = CreateWindowEx(exstyle, Get_Atl_Cls() , progid, style, x, y, w, h, _
               h_parent, NULL, GetmoduleHandle(0), NULL)
         setwindowtext h1, name1
	...
	' para evitar la duplicidad en OCIDL.Bi del FB
      #undef IClassFactory2_CreateInstanceLic
      #undef IClassFactory2_GetLicInfo
      #undef IClassFactory2_RequestLicKey
      #undef IClassFactory2_Release
...
'               byval strLicKey AS lpwstr) AS LONG
   ...
   Function AxDllGetClassObject(ByVal hdll As HMODULE, byval CLSIDS As string, byval IIDS As string, _
      ...
      axscode = fDllGetClassObject(@ClassID, cast(IID ptr,@IID_IClassFactory), @picf)
      If axscode = s_ok Then
...
'				if sclass <> Get_Atl_Cls() THEN exit function
   ...
   'CONST DISPATCH_PROPERTYPUTREF = 8  ' The function is invoked using a property reference assignment syntax.
	...
	#undef IDispatch_GetIDsOfNames
   #undef IDispatch_Invoke
   ...
   End Function
	...
	Function stock_ev(ByRef pstr As String = "")As String
	...
	Function Events_IDispatch_QueryInterface (ByVal pCookie As Events_IDispatchVtbl Ptr, ByVal riid As IID Ptr, ByVal ppVObj As PVOID Ptr) As HRESULT
	...
	Function  Ax_Events_cnx (strEvent As String, ByVal pIUnk As Any Ptr, _
   ...
   Function ToBSTR(cnv_string As String) As BSTR
      ...
      if n < 1 THEN
         ...
         return sb
      END IF
      n = (MultiByteToWideChar(CP_ACP, MB_PRECOMPOSED, cast(byte ptr,@cnv_string), - 1, NULL, 0)) - 1
		sb = SysAllocStringLen(sb, n)
		MultiByteToWideChar(CP_ACP, MB_PRECOMPOSED, cast(byte ptr,@cnv_string), - 1, sb, n)
      Return sb
   End Function
...
type VEC3D
   ...
   End Function
	...
	Function AxRegMode( filePath As String, Mode As integer=1)as integer
	  Dim hLib As HMODULE
   ...
   '***************************************************************
	...
	Function Ev_Vtbl_Release(Byval pThis As Any ptr) AS Ulong
		Function = 0
	End Function

	Function Ev_Vtbl_AddRef(Byval pThis As Any ptr) AS Ulong
		Function = 1
	End Function

	sub stock_var(byref pstr as string , byref pvtb as dword, byval rw as integer=0)
		static dstr as string
	...
	Function Ev_Vtbl_Cp(ByVal punk as any ptr, str_ev as string ,byval pSEvent as dword, byval dwconn as dword = 0) as Dword
	...
	FUNCTION Ev_Vtbl_QueryInterface(Byval pThis As Any ptr, Byref riid As GUID, Byref ppvObj As Dword) As HRESULT
		Dim riids As lpolestr
